...
__global__ void Execute(rtree_search *output,int offset,int * num_base,long int * prefix_base,gpc_vertex * vertice_base,int *num_overlay,long int * prefix_overlay, gpc_vertex * vertice_overlay,int num,gpc_polygon ** result){
	...
	if(thid<num){
		list_base.num_vertices=num_base[output[thid].nodeid-offset];
		list_base.vertex=&(vertice_base[prefix_base[output[thid].nodeid-offset]]);
		base.num_contours=1;
		base.contour=&list_base;
		
		list_overlay.num_vertices=num_overlay[output[thid].queryid];
		list_overlay.vertex=&(vertice_overlay[prefix_overlay[output[thid].queryid]]);
...
void readData(char * base_filename,char * overlay_filename, int **rect1,int **rect2,int **rect3,int **rect4,int * num_contours,int **rect1_query,int **rect2_query,int **rect3_query,int **rect4_query,int *numOfQuerys,int ** num_base,long int **prefix_base,gpc_vertex ** vertex_base,int **num_overlay,long int **prefix_overlay,gpc_vertex **vertex_overlay,int *minX,int *minY){
	...
	int i,j;
  	...
  	//MALLOC(num_base_temp, num_contours_temp * sizeof(int),"num base array creation", int);
  	//MALLOC(hole_base, num_contours_temp * sizeof(int),"hole base array creation", int);
  	//MALLOC(prefix_base_temp, num_contours_temp * sizeof(long int),"prefix base array creation", long int);
  	//MALLOC(contour_base, num_contours_temp * sizeof(gpc_vertex_list), "base contour creation", gpc_vertex_list);
    ...
    rect4_temp=(int *)malloc(num_contours_temp*sizeof(int));
  	...
  	//MALLOC(rect1_query_temp, numOfQuerys_temp * sizeof(int),"rect1_query creation", int);
	//MALLOC(rect2_query_temp, numOfQuerys_temp * sizeof(int),"rect2_query creation", int);
	//MALLOC(rect3_query_temp, numOfQuerys_temp* sizeof(int),"rect3_query creation", int);
	//MALLOC(rect4_query_temp, numOfQuerys_temp* sizeof(int),"rect4_query creation", int);
  	
  	
  	/*
  	MALLOC(num_overlay_temp, numOfQuerys_temp * sizeof(int),"num overlay array creation", int);
  	MALLOC(hole_overlay, numOfQuerys_temp * sizeof(int),"hole overlay array creation", int);
  	MALLOC(prefix_overlay_temp, numOfQuerys_temp * sizeof(long int),"prefix overlay array creation", long int);
  	MALLOC(contour_overlay, numOfQuerys_temp* sizeof(gpc_vertex_list), "overlay contour creation", gpc_vertex_list);
  	*/
	...
	for (i=0;i<num_contours_temp;i++)
  	{
    	...
    	//MALLOC(contour_base[i].vertex, num_base_temp[i] * sizeof(gpc_vertex), "vertex creation", gpc_vertex);
    	fscanf(basefp, "%lf %lf", &(temp1),&(temp2));
    	fscanf(basefp, "%lf %lf", &(temp3),&(temp4));
    	rect1_temp[i]=(int)(temp1);
    	rect2_temp[i]=(int)(temp2);
    	rect3_temp[i]=(int)(temp3);
    	rect4_temp[i]=(int)(temp4);
    	//if((rect1_temp[i]<0)||(rect2_temp[i]<0)||(rect3_temp[i]<0)||(rect4_temp[i]<0)){
    	//	printf("(%d,%d,%d,%d)\n",rect1_temp[i],rect2_temp[i],rect3_temp[i],rect4_temp[i]);
    	//}
	...
	//MALLOC(vertex_base_cpu, (num_base_temp[num_contours_temp-1]+prefix_base_temp[num_contours_temp-1]) * sizeof(gpc_vertex),"vertex array creation", gpc_vertex);
	...
	for (i=0;i<numOfQuerys_temp;i++)
  	{
    	...
    	MALLOC(contour_overlay[i].vertex, num_overlay_temp[i] * sizeof(gpc_vertex), "vertex creation", gpc_vertex);
	...
	MALLOC(vertex_overlay_cpu, (num_overlay_temp[numOfQuerys_temp-1]+prefix_overlay_temp[numOfQuerys_temp-1]) * sizeof(gpc_vertex),"vertex array creation", gpc_vertex);	
